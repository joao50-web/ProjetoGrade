...
function AppLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (["/pessoas", "/usuarios", "/cargos", "/logs"].some(p => path.startsWith(p))) {
      setOpenKeys(["admin"]);
      return;
    }
  ...
  const menuItems = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        label: "Acadêmico",
        children: [
          { key: "/academico/departamentos", icon: <ApartmentOutlined />, label: "Departamentos" },
          { key: "/academico/disciplinas", icon: <BookOutlined />, label: "Disciplinas" },
          { key: "/academico/cursos", icon: <BookOutlined />, label: "Cursos" },
        ],
      },
      { type: "divider" },
      { key: "/grade-horaria", icon: <CalendarOutlined />, label: "Grade Horária" },
  ...
  return (
    <Layout style={{ minHeight: "100vh" }}>
      <Header style={{ background: "#093e5e", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px", height: 44, position: "relative" }}>
      ...
      <Layout>
        <Sider collapsible collapsed={collapsed} trigger={null} width={200} collapsedWidth={65} style={{ background: "#ffffff", borderRight: "1px solid #e5e7eb" }}>
          <Menu mode="inline" items={menuItems} selectedKeys={[location.pathname]} openKeys={collapsed ? [] : openKeys} onOpenChange={onOpenChange} onClick={handleMenuClick} inlineCollapsed={collapsed} motion={false} style={{ border: "none", fontSize: 13, paddingTop: 6 }} />
        </Sider>
        <Content style={{ padding: "15px 25px", background: "#f5f7fa" }}>
          <Title level={4} style={{ color: "#093e5e", marginBottom: 10 }}>{currentPage}</Title>
          <div style={{ background: "#fff", padding: 20, borderRadius: 8, border: "1px solid #e5e7eb" }}>{children}</div>
        </Content>
      </Layout>
      <Footer style={{ textAlign: "center", background: "#093e5e", color: "#fff", fontSize: 10, padding: "6px" }}>© 2026 – UFCSPA</Footer>
    </Layout>
  );
}
